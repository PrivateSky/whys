...
function sendSummaruyOfExecutions(logger){
    contexts.forEach(function(context){
        logger.logWhy(JSON.stringify(context.getExecutionSummary(),null,2));
    })
}


process.on('exit',function(){
    sendSummaruyOfExecutions({
        'logWhy':function(x){console.log(x)}
    })
})
...
Function.prototype.why = function(motivation, caller,otherContextInfo, externalBinder){
    ...
    function maybeDump(context){
        if(globalWhyStackLevel === 0 && !process.env['HIDE_WHY_EXECUTION']){
