...
assert.callback("Sync and Async Embeddings", function(end) {
    ...
    function callback(){
        ...
        assert.equal(executionSummary.calls["First Call"].calls.hasOwnProperty('Motivation1 AND Motivation2'),true);
        assert.equal(executionSummary.calls["First Call"].calls.hasOwnProperty('Callback Motivation1 AND Callback Motivation2'),true);
        end();
    }
